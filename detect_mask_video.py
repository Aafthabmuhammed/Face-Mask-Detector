...
def detect_and_predict_mask(frame, faceNet, maskNet):
    ...
    for i in range(0, detections.shape[2]):
        ...
        # filtering out weak detections by ensuring the probability is
    ...
    if len(faces) > 0:
        # for faster inference we are making batch predictions on *all*
...
while True:
    # resizing videostream to have a maximum width of 400 pixels
    frame = vs.read()
    frame = imutils.resize(frame, width=400)

    # detecting faces in the frame and determining if they are wearing a
    # face mask or not
    ...
    for (box, pred) in zip(locs, preds):
        ...
        # determining the class label and color we'll use to draw
    ...
    # if the `q` key is pressed we will break from the loop
    if key == ord("q"):
        break
